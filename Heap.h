...
private:
	...
	void percolateDown(unsigned int hole)
	{
		...
		while (true)
		{ // loop until hole is at correct position or leaf node
			int leftchild = 2 * hole;
			if (leftchild > _items.size() - 1) // hole is leaf node
			{
				break;
			}
			int rightchild = 2 * hole + 1;
			// move hole to smallest value of left and right child, assuming both children are smaller than current hole
			int newchild;
			if (leftchild == _items.size() - 1 || _items[leftchild] < _items[rightchild])
			{ // only has left child or left value is smaller than rights value
				newchild = leftchild;
			}
			else
			{
			...
			// if item at hole is less than child then already at correct position
			if (_items[hole] < _items[newchild])
			{
				break;
			}
			//otherwise move the hole down
			else
			{
				auto temp = _items[hole];
				_items[hole] = _items[newchild];
				_items[newchild] = temp;
				hole = newchild;
			}
		}
	}
	...
	void percolateUp(T item)
	{
		...
		// move hole up if holes value is less than parents value
		while (_items[hole] < _items[floor(hole / 2)])
		{
			int parent = floor(hole / 2);
			auto temp = _items[parent];
			_items[parent] = _items[hole];
			_items[hole] = temp;
			hole = parent;
		}
	}
